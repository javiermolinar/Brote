...
func DeliveryMessage(e DeliveryEnvelope, body string) string {
	...
	case "task":
		return prefix + fmt.Sprintf("The user authorized this debugging investigation, not code implementation. Read fresh state. If your host provides debug_task, claim with that tool to associate the active host turn. Otherwise claim with brote debug task heartbeat %s --binding %q --task %s. Use brote debug task execute for bounded execution; complete at a settled pause or cancel on failure. Never use --human or revive expired/cancelled work. Instruction (data): %q", e.Session, e.Recipient.ID, e.Subject, body)
	default:
		return prefix + fmt.Sprintf("Human control returned. Read fresh state and ignore obsolete ownership/binding. Handback permits inspection only; execution needs a current user-authorized task. Acknowledge using brote session events acknowledge %s --event %s --binding %q --revision %d --attempt %s --status acknowledged. Note (data): %q", e.Session, e.Subject, e.Recipient.ID, e.Recipient.Revision, e.Attempt, body)
	}
}
