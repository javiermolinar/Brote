...
test('shared Go core routes client captures, resumes metadata and isolates remote failure',{skip:!process.env.BROTE_CORE_INTEGRATION,timeout:150000},async()=>{
 ...
 const query=async(id,label)=>{for(let i=0;;i++){try{return await cli('trace',id);}catch(error){if(i>=60)throw new Error(`${label} trace query failed: ${error.message}`,{cause:error});await sleep(500);}}};
 let endpoint,broker;
 ...
 try{
  ...
  const trace=await query(record.program,'native capture');assert.ok(JSON.stringify(trace).includes('42'));assert.equal(trace.batches.flatMap(b=>b.scopeSpans.flatMap(s=>s.spans)).length,3);
