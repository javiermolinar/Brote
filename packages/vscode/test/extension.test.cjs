...
const source={native:compile('native.ts'),extension:compile('extension.ts')};
...
async function harness({enabled=false,bundle,exportEnv,traceAction='Copy ID'}={}){
 ...
 const loader=n=>{runtimeRequires.push(n);if(n==='vscode')return api;if(n==='./native')return nativeModule;if(n==='./coreTrace')return core;if(bundle && !builtinModules.includes(n)&&!n.startsWith('node:'))throw new Error('Unbundled dependency: '+n);return req(n);};
