...
Share a live Delve debugging session between an LLM agent and a human. The current implementation ships as **Debug Handover**, with a Codex skill, a shared browser inspector, and Zed / VS Code integration. The planned harness adapters, including Pi, are described in [ARCHITECTURE.md](ARCHITECTURE.md).
...
npm ci
npm run build
(cd vscode && npm exec --yes --package=@vscode/vsce@4.0.0 -- vsce package --no-dependencies --allow-missing-repository --skip-license --out debug-handover-0.1.0.vsix)
```

Source is in `vscode/src`; its bundled output requires no Node installation for normal use. Protocol tests cover fresh request admission, stale request suppression, local endpoint validation, API errors, and redirect rejection. Real Delve integration tests exercise both Zed and VS Code ownership, stepping, handback, and safe disconnect.
...
The inspector is written in strict TypeScript at `web/app.ts`. Highlight.js supplies Go syntax colors, with a separate gutter and execution-line overlay. Build the committed browser bundle with Node.js and npm:
...
`web/app.js` is the generated bundle embedded by Go. Rebuild it after changing TypeScript. Node.js is only needed for frontend development. The Highlight.js license is included in `THIRD_PARTY_NOTICES.md`.
...
Package a shareable archive without targets, session tokens, logs, or node_modules:
